...
int CVirtualClient2Dlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pDlgLog = new CDlgLog(this);
	m_pDlgLog->Create( CDlgLog::IDD, this );
	m_pDlgLog->ShowWindow(TRUE);
...
void CVirtualClient2Dlg::OnSize(UINT nType, int cx, int cy)
{
	...
	const int treeCtrlW = (int)(cx * 0.4f);
	const int treeCtrlH = (int)(cy * 0.6f);

	if (m_ProtocolTree.GetSafeHwnd())
		m_ProtocolTree.MoveWindow(CRect(0,0,treeCtrlW,treeCtrlH));
	if (m_pDlgLog)
		m_pDlgLog->MoveWindow(CRect(0, treeCtrlH+5,cx,cy));
}
